...
# api_keys = eval(config.get("openai", "api_keys")) # GPT API key will be store in os.environ["OPENAI_API_KEY"]
gpt_model = config.get("openai", "model")
gpt_temperature = eval(config.get("openai", "temperature"))
gpt_top_p = eval(config.get("openai", "top_p"))
gpt_frequency_penalty = eval(config.get("openai", "frequency_penalty"))
gpt_presence_penalty = eval(config.get("openai", "presence_penalty"))
